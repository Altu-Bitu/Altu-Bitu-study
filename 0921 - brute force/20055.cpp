...
void second(int n, int start, int end) {
    int cur = end; // end 값 cur 에 저장
    while (cur != start) { //end - 1 부터 start 까지 검사
        cur = minusPosition(n, cur); //인덱스 감소 시키기
        ...
        if (belt[cur].is_on && !belt[next].is_on && belt[next].power) { //현재 위치에 로봇이 있고, 다음 위치에 로봇이 없으며 내구성 1이상인 경우 -> 로봇 옮김
            ...
            if (belt[next].power == 0) //내구성 0이면 zero_power 값 높이기
                zero_power++;
        }
    }
}
...
int main() {
    ...
    //입력
    cin >> n >> k;
    belt.assign(n * 2, {0, false}); //벨트 초기화 false로 할당
    for (int i = 0; i < n * 2; i++) //벨트 크기 만큼. belt의 내구도 power값 입력받기
        cin >> belt[i].power;
    ...
    int step = 0;     //단계 수
    while (true) {
        start = minusPosition(n, start); // n, 0 값으로 함수호출 하고 값 할당
        end = minusPosition(n, end); //n, n-1값으로 함수호출 하고 값 할당
        if (belt[end].is_on) //벨트 회전 후, 로봇이 내리는 위치에 있다면 내리기
            belt[end].is_on = false; //true면 false로 값 넣기
        second(n, start, end);
        third(start);
        step++; // 반복문 돈만큼 step 값 증가
        if (zero_power >= k) // 내구성이 0인 값이 k이상이면 반복문 탈출하기
            break;
    }
