...
int calcMode(vector<int> &frequency) {
    ...
    for (int i = 0; i <= SIZE * 2; i++) { //최빈값의 빈도수 구하기
        if (frequency[i] > max_value) //빈도수가 max_value 보다 크다면
            max_value = frequency[i]; // 빈도수값을 max_value로
    }
    //두 번째로 작은 최빈값 구하기
    int cnt = 0;  //최빈값 개수
    int mode = 0; //최빈값
...
int main() {
    ...
    vector<int> num(n); // n개만큼 크기 할당하여 Vector 선언
    vector<int> frequency(SIZE * 2 + 1, 0); //인덱스는 입력 수 + 4000의 값. 따라서 0 ~ 8000. 각 인덱스에 해당 수가 얼마나 나왔는지 저장

    double sum = 0; //평균 구하기 위한 합 변수
    for (int i = 0; i < n; i++) {
        cin >> num[i]; // n번 만큼 입력 받기
        sum += (double) num[i]; //합계 구하기
        frequency[num[i] + SIZE]++; // 입력수+4000의 인덱스값 증가
    }
