...
//유레카인지 검사하는 반복문 함수
bool eureka(int num) { // 입력받은 수 넘기기
    for (int i = 1; i <= 45; i++) { //반복하여, 3개의조합으로 다 더해보기
        for (int j = 1; j <= 45; j++) {
            for (int k = 1; k <= 45; k++) {
                if (tri_num[i] + tri_num[j] + tri_num[k] == num) //삼각수 3개 조합 값이 입력받은수면?
                    return true; //true 로 리턴하기!
            }
        }
    }

    return false;// 해당 사항 없으면 false 로 리턴하기
}
