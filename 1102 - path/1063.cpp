//
// Created by banya on 2021-11-09.
//
...
pair<ci, ci> moveKing(string move, ci point_k, ci point_r) {
    ...
    int king_nr = point_k.first + dr[d]; //dr의 방향 인덱스값 더하여 킹 위치 저장
    int king_nc = point_k.second + dc[d];//dc의 방향 인덱스값 더하여 킹 위치 저장
    ...
    //돌이 있다면
    int rock_nr = point_r.first + dr[d];//dr의 방향 인덱스값 더하여 돌 위치 저장
    int rock_nc = point_r.second + dc[d];//dc의 방향 인덱스값 더하여 돌 위치 저장
...
int main() {
    ...
    pair<ci, ci> point = {point_k, point_r}; //킹 돌 위치 페어로 저장
    while (n--) {
        cin >> move;
