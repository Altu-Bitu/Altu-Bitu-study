//
// Created by banya on 2021-10-05.
//
...
int easyStairNum(int n) {
    ...
    for (int i = 2; i <= n; i++) { //i: 자릿수
        for (int j = 1; j <= 10; j++) { //j: 끝나는 수 (0 ~ 9)
            //점화식 세우기
            //해당 자릿수의 현재 끝나는 수 = 이전 자릿수의 이전 끝나는 수 + 이전자릿수의 다음 끝나는 수를 더한값
            // 1000000000 을 나눈 나머지의 값
            dp[i][j] = (dp[i - 1][j - 1] + dp[i - 1][j + 1]) % 1000000000;
        }
    }
    //해당 자릿수의 1~10로 끝나는 쉬운 계단수 합하면서 100000000나머지 저장
