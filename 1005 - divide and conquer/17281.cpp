...
int calcScore() {
    ...
    for (int i = 0; i < n; i++) { //i: 이닝
        ...
        int out = 0; //아웃 선수 카운트
        while (out < 3) { //카운트가 3보다 작으면 반복
            int state = innings[i][order[idx++]]; //현재 선수의 공격 상태
            if (idx == 10)  // 인덱스 번호가 10이면
                idx = 1; //1 대입
            switch (state) { //공격 상태에 따라 점수 계산
                case 0:
                    out++;
                    break;
                case 1: //안타, 1루씩 진루
                    score += baseman[3]; //3루에 있는 선수있다면 홈 도착
                    for (int i = 3; i > 0; i--) // 3번 반복하여서
...
void array(int cnt) { //cnt: 타자순서
    ...
    if (cnt == 10) { //9번 타자까지 정해짐 (기저조건)
        ...
        ans = max(ans, score); //ans값과 호출해서 넣은 score값중 max 구한후 ans대입
        return;
    }
    for (int i = 2; i <= 9; i++) {
        if (!check[i]) { //false면
...
// Created by banya on 2021-10-12.
//
